...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      // notify parent at the moment cards start promoting so content can update earlier
      tl.call(
        () => {
          const newFront = rest[0] ?? front;
          onActiveChange?.(newFront ?? 0);
        },
        undefined,
        'promote',
      );
    ...
    // initial notify: current front is order.current[0]
    onActiveChange?.(order.current[0] ?? 0);
